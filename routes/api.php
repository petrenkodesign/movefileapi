...
Route::middleware('api_token')->get('/movefiles', function (Request $request) {
    ...
    $command = 'rsync -avP --ignore-existing -e "ssh -i '.env('SERVER_SSHKEY').'" '.env('SERVER_POINT_USER').'@'.env('SERVER_SRC_IP').':'.env('SERVER_SRC_DIR').env('FILES_MASK').' '.env('SERVER_POINT_DIR').' 2>&1';
    exec($command, $output);
